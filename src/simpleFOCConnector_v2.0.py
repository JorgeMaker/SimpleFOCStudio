...
class SimpleFOCDevice:
    ...
    def pushConfiguration(self):
        ...
        self.sendRamp(self.voltageRampPID)
...
class SerialPortReceiveHandler(QtCore.QThread):
    monitoringDataReceived = QtCore.pyqtSignal(float, float, float)
    ...
    def handle_received_data(self, data):
        if self.isDataReceivedMonitoring(data):
            try:
                ...
                self.monitoringDataReceived.emit(float(v[2]), float(v[1]),
                                                float(v[0]))
